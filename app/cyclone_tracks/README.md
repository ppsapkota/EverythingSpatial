...
Then open <http://127.0.0.1:8899/app/cyclone_tracks/>.

## Deploy (GitHub Pages)

The app is self-contained under `app/cyclone_tracks/`: `index.html` loads its data via
`./data/...` relative paths, so publish **`app/cyclone_tracks/` as the Pages root** and
the page ships with its data. Nothing under the repo's `data/` (raw/processed/curated)
is published.
...
app/cyclone_tracks/data/           what the app loads                <product>.geojson
app/cyclone_tracks/index.html      the web app
```

The app-delivery layer sits under `app/<domain>/` (not `data/`) on purpose: each
domain is then a self-contained web app, so publishing `app/cyclone_tracks/` as a
GitHub Pages root ships `index.html` and its `data/` together via `./data/...`
relative paths, and the large raw/processed layers under `data/` never get published.
`paths.app_data_dir()` points here.

Which script writes where: `preprocess_ibtracs.py` → 01_raw + 02_processed + app layer.
`make_elnino_layers.py` → 03_curated + app layer. A season-floored subset is still a
...
pipeline script. Parquet never goes to the app layer: MapLibre has no Parquet
...
**The app layer is deliberately unstamped** — `index.html` references those filenames
directly, so a changing name would break the app on every regeneration. It holds the
current copy under a stable name; history lives in 02/03.
...
appears solely in the app layer.
...
- **Full archive** → `data/02_processed/cyclone_tracks/IBTrACS.since1980...lines_<ts>.gpkg`
  (75 MB, all seasons) for QGIS/analysis.
